...
public class Battle {
    ...
    public void 일반공격(Player player, Monster monster){
        ...
        monster.체력 -= 데미지;
        GA.화면클리어();
        System.out.println("전투에서 당신은 " + monster.이름 + "에게 " + 데미지 + " 피해를 주었고, 당신은 " + monster.이름 + "에게 " + 받은데미지 + " 피해를 받았다!!!");
        GA.중간띄우기();
        GA.아무키나();
    }
    public void 마법공격(Player player, Monster monster, int 마법입력){
        GA.화면클리어();
        GA.정보알림("                "+player.마법주문슬롯.get(마법입력 - 1).이름 + "!!!");
        GA.중간띄우기();
        GA.아무키나();
        if (player.마력 >= player.마법주문슬롯.get(마법입력 - 1).마력소모) {
            GA.정보알림(player.마법주문슬롯.get(마법입력 - 1).시전);
            int 데미지 = player.마법주문슬롯.get(마법입력 - 1).데미지 - monster.방어();
            int 받은데미지 = monster.일반공격() - player.방어();
            ...
            player.체력 -= 받은데미지;
            player.마력 -= player.마법주문슬롯.get(마법입력 - 1).마력소모;
            monster.체력 -= 데미지;
